...
const containerListAssetPath = './assets/figma-container-list-38-31369';
...
function instanceMarkup(pod,tab='detail'){
  ...
  const containerProfiles=[
    ...
    {name:'ranking-inference',type:'Sidecar',tone:'sidecar',status:'运行中',ready:'是',restarts:'0',age:'8d',image:'registry.internal/observability/metrics-exporter:v1.8.2',pull:'IfNotPresent',command:'/exporter --listen=:9100',cpu:['0.3c','1c','0.5c',30],memory:['0.48Gi','2Gi','1Gi',24],ports:[['静态','metrics','9100']],mounts:[['config','/etc/exporter','ConfigMap: metrics-config','只读']],env:[['SCRAPE_INTERVAL','15s','Config'],['METRICS_PATH','/metrics','Config']],termination:['Completed','0','2026.06.01 13:20:12','2026.06.01 13:20:13']},
    {name:'ranking-inference',type:'普通',tone:'regular',status:'运行中',ready:'是',restarts:'0',age:'6d',image:'registry.internal/payments/feature-cache:v1.4.0',pull:'Always',command:'/cache/server --port=8600',cpu:['1.2c','2c','2c',60],memory:['6.4Gi','8Gi','4Gi',80],ports:[['静态','cache','8600']],mounts:[['cache','/var/cache/features','PVC: feature-cache','读写']],env:[['CACHE_SIZE','8Gi','Config'],['CACHE_NAMESPACE','ranking','Config']],termination:['Completed','0','2026.05.30 08:12:40','2026.05.30 08:12:41']}
  ];
  ...
  const containers=`<nav class="container-strip ${state.containerListCollapsed?'is-collapsed':''}" aria-label="容器选择"><div class="container-strip-list">${containerProfiles.map((item,index)=>`<button class="${state.selectedContainer===index?'active':''}" data-container-select="${index}" ${state.selectedContainer===index?'aria-current="true"':''}>${index<3?`<img class="container-dot" src="${containerListAssetPath}/${containerDotAssets[index]}" alt="">`:`<img class="container-dot" src="${containerListAssetPath}/image_3.png" alt="">`}<strong>${item.name}</strong><small>${item.type}</small></button>`).join('')}</div><button class="container-list-toggle" type="button" data-container-list-toggle aria-expanded="${!state.containerListCollapsed}" aria-label="${state.containerListCollapsed?'展开全部容器':'收起容器列表'}"><img src="${containerListAssetPath}/image_9.png" alt=""></button></nav>`;
...
function openInstanceDetail(id,tab='detail'){ const pod=pods.find(item=>item[0]===id); if(!pod)return; if(state.activeInstanceId!==id){state.selectedContainer=0;state.containerListCollapsed=false;state.activeInstanceId=id;} instanceModal.innerHTML=instanceMarkup(pod,tab); detailBackdrop.classList.remove('hidden'); }
function closeInstanceDetail(){ detailBackdrop.classList.add('hidden'); instanceModal.innerHTML=''; state.containerListCollapsed=false; }
...
instanceModal.addEventListener('click',event=>{
  ...
  const containerListToggle=event.target.closest('[data-container-list-toggle]');
  if(containerListToggle&&body){const activeTab=instanceModal.querySelector('[data-detail-tab].active')?.dataset.detailTab || 'detail';state.containerListCollapsed=!state.containerListCollapsed;openInstanceDetail(body.dataset.instanceId,activeTab);return;}
