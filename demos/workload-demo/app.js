...
function renderApplicationMenu(query='',focusInput=false){
  ...
  menu.innerHTML=`<h3 class="application-menu-title">配置应用层级</h3><label class="application-menu-search">${icon('search')}<input data-application-search aria-label="搜索应用" placeholder="搜索应用"></label><div class="application-menu-tabs"><button data-application-tab="all">可用应用</button><button class="is-active" data-application-tab="favorite">收藏应用</button><button data-application-tab="recent">最近访问</button><button class="application-menu-open-list" data-application-open-list>打开应用列表</button></div><div class="application-menu-list">${listMarkup}</div><div class="application-menu-divider"></div><div class="application-menu-footer"><button data-application-create>${icon('add')}新建应用</button><button data-application-permission>${icon('clipboard')}申请应用权限</button></div>`;
